...
(define-key lisp-mode-shared-map (kbd "β") 'cd-package-symbol)
...
(defun cd-lisp-interaction-hook ()
  (lispy-mode 1)
  (company-mode 1))

;;* Add the last eval result to "M-y"
(defvar cd-last-eval-expression-result "")

(defun cd-last-eval-expression-result ()
  (list cd-last-eval-expression-result))

(defun cd-eval-expression (orig-fun &rest args)
  (setq cd-last-eval-expression-result
        (prin1-to-string
         (apply orig-fun args))))
(cd-advice-add 'eval-expression :around #'cd-eval-expression)

(ivy-set-sources
 'counsel-yank-pop
 '((original-source)
   (cd-last-eval-expression-result)))
...
(defun cd-edebug-hook ()
  ;; (lispy-mode -1)
  )

(add-hook 'edebug-setup-hook 'cd-edebug-hook)
